...
```json
{
  "clients": {
    "zeus": {
      "command": ["/path/to/zeus", "lsp", "--stdio"],
...
1. Ensure the `zeus` binary exists:
   ```bash
   make build
   ls -la ./zeus
   ```
...
- [IDE Setup](/getting-started/ide-setup/) — Detailed VS Code setup instructions
- [Compiler](/tooling/compiler/) — Command-line compilation
