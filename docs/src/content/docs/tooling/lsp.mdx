...
The server implements `textDocument/formatting` using the built-in
[formatter](/tooling/formatter/), so **Format Document** (`Shift+Alt+F`) and
...
The server never returns an edit for code that doesn't parse, so a file with
...
Now implemented — see [Formatting](#formatting) above.
