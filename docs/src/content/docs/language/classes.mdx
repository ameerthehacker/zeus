...
work on any object out of the box. By default an object prints structurally (`ClassName { field:
value }`). Define a public `toString(): string` to customize it — it's used everywhere the object
becomes a string, including nested inside other objects:

```zeus
class Temperature {
  celsius: f64;
  constructor(celsius: f64) { this.celsius = celsius; }
  toString(): string { return this.celsius + "°C"; }
}

let t: Temperature = new Temperature(21.5);
console.log(t);             // 21.5°C
console.log("now: " + t);  // now: 21.5°C
```

See [Console](/language/console/) for the full details of how values render.
...
team.addScore(20);
team.addScore(30);
console.log(team.getScore(1));   // 20
```
