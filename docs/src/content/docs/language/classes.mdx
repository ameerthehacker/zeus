---
title: Classes
description: Object-oriented programming with classes in Zeus.
---

import { Aside } from '@astrojs/starlight/components';

Classes in Zeus provide a way to create custom types with properties and methods.

## Basic Class

```zeus
class Rectangle {
  public width: f64;
  public height: f64;
}

let rect: Rectangle = new Rectangle();
rect.width = 10.0;
rect.height = 5.0;
```

## Constructors

Use `constructor` to initialize objects:

```zeus
class Point {
  public x: i32;
  public y: i32;

  constructor(x: i32, y: i32) {
    this.x = x;
    this.y = y;
  }
}

function main(): i32 {
  let p: Point = new Point(10, 20);
  return p.x + p.y;  // Returns 30
}
```

<Aside type="note">
  The constructor is called automatically when you use `new ClassName(...)`.
</Aside>

## Methods

Classes can have methods that operate on the object:

```zeus
class Rectangle {
  public width: f64;
  public height: f64;

  constructor(w: f64, h: f64) {
    this.width = w;
    this.height = h;
  }

  public area(): f64 {
    return this.width * this.height;
  }

  public perimeter(): f64 {
    return 2.0 * (this.width + this.height);
  }
}

let rect: Rectangle = new Rectangle(10.0, 5.0);
let a: f64 = rect.area();       // 50.0
let p: f64 = rect.perimeter();  // 30.0
```

Methods support [rest parameters](/language/functions/#rest-parameters) too — prefix the final parameter with `...`:

```zeus
class Accumulator {
  public base: i32;

  constructor(base: i32) {
    this.base = base;
  }

  public sum(...nums: i32[]): i32 {
    let total: i32 = this.base;
    for (let i: i32 = 0; i < nums.length; i++) {
      total += nums[i];
    }
    return total;
  }
}

let acc: Accumulator = new Accumulator(10);
let total: i32 = acc.sum(1, 2, 3);  // 16
```

## The `this` Keyword

Inside a class, `this` refers to the current instance:

```zeus
class Counter {
  public value: i32;

  constructor(start: i32) {
    this.value = start;
  }

  public increment() {
    this.value = this.value + 1;
  }

  public add(amount: i32) {
    this.value = this.value + amount;
  }

  public getValue(): i32 {
    return this.value;
  }
}
```

## Access Modifiers

Zeus supports `public` and `private` access modifiers:

```zeus
class BankAccount {
  private balance: i32;
  public accountId: i32;

  constructor(id: i32, initialBalance: i32) {
    this.accountId = id;
    this.balance = initialBalance;
  }

  public deposit(amount: i32) {
    this.balance = this.balance + amount;
  }

  public getBalance(): i32 {
    return this.balance;
  }
}

function main(): i32 {
  let account: BankAccount = new BankAccount(1, 100);
  account.deposit(50);
  
  // account.balance = 0;  // Error: 'balance' is private
  
  return account.getBalance();  // 150
}
```

## Nested Objects

Classes can contain other objects as properties:

```zeus
class Point {
  public x: i32;
  public y: i32;

  constructor(x: i32, y: i32) {
    this.x = x;
    this.y = y;
  }
}

class Line {
  public start: Point;
  public end: Point;

  constructor(p1: Point, p2: Point) {
    this.start = p1;
    this.end = p2;
  }

  public length(): i32 {
    let dx: i32 = this.end.x - this.start.x;
    let dy: i32 = this.end.y - this.start.y;
    return dx + dy;  // Simplified (Manhattan distance)
  }
}

function main(): i32 {
  let p1: Point = new Point(0, 0);
  let p2: Point = new Point(10, 20);
  let line: Line = new Line(p1, p2);
  
  return line.length();  // Returns 30
}
```

## Object References

Objects are passed by reference:

```zeus
class Box {
  public value: i32;

  constructor(v: i32) {
    this.value = v;
  }
}

function modifyBox(box: Box) {
  box.value = 999;
}

function main(): i32 {
  let myBox: Box = new Box(10);
  modifyBox(myBox);
  
  return myBox.value;  // Returns 999
}
```

## Null Objects

Object variables can be `null`:

```zeus
class Node {
  public value: i32;
  public next: Node;

  constructor(v: i32) {
    this.value = v;
    this.next = null;  // No next node
  }
}

function main(): i32 {
  let node: Node = new Node(42);
  // node.next is null
  
  return node.value;
}
```

### Null Reference Checking

Zeus automatically checks for null when accessing object properties. Attempting to access a property on a null object throws a `NullReferenceException`:

```zeus
class Person {
  public name: string;
  constructor(name: string) {
    this.name = name;
  }
}

let person: Person;       // null by default
console.log(person.name);         // Throws NullReferenceException!
```

<Aside type="caution">
  Always ensure objects are initialized before accessing their properties, or use try-catch to handle potential null reference errors.
</Aside>

### Handling Null Safely

You can use try-catch to handle null reference errors:

```zeus
function getPersonName(person: Person): string {
  try {
    return person.name;
  } catch (e: Error) {
    return "Unknown";
  }
}
```

## Method Chaining

Methods can return `this` for chaining (when returning the same type):

```zeus
class Builder {
  public x: i32;
  public y: i32;
  public z: i32;

  constructor() {
    this.x = 0;
    this.y = 0;
    this.z = 0;
  }

  public setX(val: i32): i32 {
    this.x = val;
    return this.x;
  }

  public setY(val: i32): i32 {
    this.y = val;
    return this.y;
  }
}
```

## Classes with Arrays

Classes can have array properties:

```zeus
class Team {
  public scores: i32[];

  constructor() {
    this.scores = new i32[];
  }

  public addScore(score: i32) {
    this.scores.push(score);
  }

  public getScore(index: i32): i32 {
    return this.scores.get(index);
  }
}

function main(): i32 {
  let team: Team = new Team();
  team.addScore(10);
  team.addScore(20);
  team.addScore(30);
  
  return team.getScore(1);  // Returns 20
}
```

---

## Anonymous Classes

A class expression can be used inline without declaring a named type. This is useful for one-off objects or when you want to encapsulate logic without polluting the namespace.

### Assign to a variable

The most common form — assign the anonymous class to a `const`, then use the const as the type:

```zeus
function main(): i32 {
  const Counter = class {
    public n: i32;

    constructor(start: i32) {
      this.n = start;
    }

    inc(): i32 {
      this.n = this.n + 1;
      return this.n;
    }
  }

  let c = new Counter(9);
  return c.inc();  // 10
}
```

### Named class expression

You can give the class an internal name. Both the variable name and the class name are then usable in the same scope:

```zeus
function main(): i32 {
  const Vec = class Vector {
    public x: i32;
    public y: i32;

    constructor(x: i32, y: i32) {
      this.x = x;
      this.y = y;
    }

    sum(): i32 { return this.x + this.y; }
  }

  let a = new Vec(3, 4);     // via the const
  let b = new Vector(1, 2);  // via the class name
  return a.sum() + b.sum();  // 10
}
```

### Inline instantiation

Use `new (class { ... })(args)` to create and instantiate an anonymous class in a single expression — no variable needed for the class itself:

```zeus
function main(): i32 {
  let obj = new (class {
    public v: i32;

    constructor(v: i32) { this.v = v; }

    get(): i32 { return this.v; }
  })(42);

  return obj.get();  // 42
}
```

This is the class equivalent of an IIFE: the class is defined and immediately instantiated. It's useful when you need a one-off object with methods but don't want to declare a named class.

---

## Next

<a href="/language/arrays/">Arrays →</a>

