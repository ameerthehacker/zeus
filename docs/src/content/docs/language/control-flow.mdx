...
The `for` loop is ideal for counting iterations:
...
- **Initializer**: Runs once before the loop starts (typically declares loop variable)
- **Condition**: Checked before each iteration; loop continues while true
- **Increment**: Runs after each iteration (typically updates loop variable)
