...
let info = `${name} v${version}`;         // "Zeus v1.0"
let plain = `no interpolation here`;      // "no interpolation here"
```

Each `${...}` expression must be of type `string`. Segments are concatenated left-to-right using the same `+` operator as explicit string concatenation, so all the same type rules apply.

<Aside type="caution" title="String expressions only">
  Interpolated expressions must be of type `string`. To embed a number, convert it to a string first using a method or format it via another string variable.
</Aside>
