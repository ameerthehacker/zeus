...
<Aside type="note">
Indices and lengths are **byte** offsets (`slice`, `indexOf`, `charAt`, …), consistent with `string`
indexing and `length`. `split("")` yields one entry per byte, so multi-byte UTF-8 characters are split
into their bytes.
</Aside>
...
Any value can be interpolated — numbers, booleans, objects, and arrays are converted to a string
automatically (via their `toString`, structural reflection for objects/arrays otherwise). The same
conversion powers `+` string concatenation:
...
<Aside type="tip">
  Define a public `toString(): string` on a class to control how it renders inside templates,
  concatenation, and `console.log` — see [Console](/language/console/).
</Aside>
...
Regular-expression matching (`match`, regex-based `replace`/`split`) is planned once Zeus gains a
regex engine.
