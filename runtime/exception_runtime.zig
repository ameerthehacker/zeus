...
/// Print error message from Error object
fn printErrorMessage(writer: anytype, obj_ptr: *anyopaque) void {
    // Access message field from Error object
    const error_obj = castToErrorObj(obj_ptr);
    
    if (error_obj.message) |msg_ptr| {
        // Cast to string object and extract the message
        const string_obj = @as(*StringObj, @ptrCast(@alignCast(msg_ptr)));
        if (string_obj.data) |array| {
            if (array.length > 0 and array.data != null) {
                const bytes = @as([*]const u8, @ptrCast(@alignCast(array.data.?)))[0..@intCast(array.length)];
                writer.print("{s}", .{bytes}) catch {};
                return;
            }
        }
    }
    
    // Fallback if message couldn't be extracted
...
/// Error constructor: zeus_error_constructor(this_ptr, return_buffer, message_ptr_ptr)
/// Called when `new Error("message")` is executed
...
export fn zeus_error_constructor(this_ptr: *anyopaque, return_buffer_ptr: ?*anyopaque, message_ptr_ptr: *anyopaque) callconv(.C) void {
    _ = return_buffer_ptr;

    const error_obj = castToErrorObj(this_ptr);
    
    // Dereference to get the string object pointer
