...
var brokenSeeds = []string{
	...
	// Interfaces, incl. malformed members and cyclic `extends` (exercises interface member walking).
	"interface",
	"interface Shape",
	"interface Shape {",
	"interface Shape { name: }",
	"interface Shape { area(): }",
	"interface Shape { name: string; area(): f64; }",
	"interface A extends {}",
	"interface A extends B {} interface B extends A {}",
	"interface S { x: i32 }\nfunction f(s: S): i32 { return s. }",
	"function f(s: Shape): i32 { return s.area(); }",
	"class C implements Shape { }",
	"let x: Shape = 0; x.area()",
	"let x: Unknown = 1",
	"let x: i32[] = [",
	"let x = () =>",
	"let x = (a) => a.",
