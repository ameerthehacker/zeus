...
// Note: Array class definitions are now pre-generated by ArrayTypeCollectionPass
func (tc *TypeChecker) getClassFromArrayType(arrayType zeus_value.ArrayType) *zeus_value.Class {
	...
	// This should never happen - ArrayTypeCollectionPass should have already created this class
	zeus_error.Assert(false, fmt.Sprintf("ArrayTypeCollectionPass did not create class for array type %s", arrayTypeClassName))
	return nil
}

// ArrayTypeCollectionPass collects all array types used in the program
// and generates class declarations for them at the top level
type ArrayTypeCollectionPass struct {
	arrayTypes map[string]zeus_value.ArrayType
}

func NewArrayTypeCollectionPass() *ArrayTypeCollectionPass {
	return &ArrayTypeCollectionPass{
		arrayTypes: make(map[string]zeus_value.ArrayType),
	}
}

func (p *ArrayTypeCollectionPass) GetName() string {
	return "ArrayTypeCollectionPass"
}

func (p *ArrayTypeCollectionPass) HandleInstruction(tc *TypeChecker, instr *Instr) {
	// Collect array types from instruction outputs
	if instr.Output != nil && instr.Output.ValueType != nil {
		p.collectArrayType(instr.Output.ValueType)
	}
	...
	case InstrTypeDeclClass:
		...
		for _, property := range input.Class.Properties {
			p.collectArrayType(property.Property.ValueType)
		}
		if input.Class.ArrayElementType != nil {
			p.collectArrayType(input.Class.ArrayElementType)
		}
	case InstrTypeDeclClassMethod:
		input := AsDeclClassMethodInstrInput(instr.Input)
		p.collectArrayType(input.Method.ReturnType)
...
func (p *ArrayTypeCollectionPass) collectArrayType(valueType zeus_value.ValueType) {
	if zeus_value.IsArrayType(valueType) {
		...
		// Recursively collect element types if they are also arrays
		p.collectArrayType(arrayType.ElementType)
	}
}

func (p *ArrayTypeCollectionPass) Finalize(tc *TypeChecker) {
	// Generate class declarations for all collected array types
	...
	for _, arrayType := range sortedTypes {
		...
		// Check if class already exists in symbol table
		if _, ok := tc.builder.symbolTable.GetSymbol(typeName); ok {
			continue
		}
		...
		// Emit DECL_CLASS instruction at the top level (before any function code)
		result := tc.builder.createTempVariable(arrayType.GetSpan())
		tc.builder.insertionIndex = 0 // Insert at the beginning
		tc.builder.currentBlock = nil  // Ensure we're not in a block
...
func (p *ArrayTypeCollectionPass) sortArrayTypesByDepth() []zeus_value.ArrayType {
...
func (p *ArrayTypeCollectionPass) calculateDepth(arrayType zeus_value.ArrayType) int {
	depth := 1
