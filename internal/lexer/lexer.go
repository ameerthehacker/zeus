...
func NewLexer(source string) *Lexer {
	return &Lexer{source: []rune(source), cursor: 0, start: 0, tokens: []*token.Token{}}
}
...
func (l *Lexer) eatIdentifierOrKeywordOrDatatype() {
	...
	identifier := string(l.source[l.start:l.cursor])

	if token.IsKeyword(identifier) {
		l.pushToken(token.NewTokenWithValue(token.TokenTypeKeyword, identifier, token.NewSpan(l.start, l.cursor - 1)))
	} else if token.IsDataType(identifier) {
		l.pushToken(token.NewTokenWithValue(token.TokenTypeDataType, identifier, token.NewSpan(l.start, l.cursor - 1)))
	} else {
		l.pushToken(token.NewTokenWithValue(token.TokenTypeIdentifier, identifier, token.NewSpan(l.start, l.cursor - 1)))
	}
}
...
func (l *Lexer) eatNumber() {
	...
	l.pushToken(token.NewTokenWithValue(token.TokenTypeNumber, string(l.source[l.start:l.cursor]), token.NewSpan(l.start, end)))
}
